...
from utils.helpers import clean_gstin
import re
import time
...
class GSTReconciliation:
    ...
    def _process_exact_matches(self):
        ...
        for key in common_keys:
            ...
            for pairing in pairings:
                ...
                is_exact_match = (
                    abs(igst_diff) <= self.tax_tolerance and
                    abs(cgst_diff) <= self.tax_tolerance and
                    abs(sgst_diff) <= self.tax_tolerance
                )
                ...
                status = 'Exact Match'
                if not is_exact_match: status = 'Partial Match'
                if cross_year:
                    status = 'Cross-Year Match' if is_exact_match else 'Partial Cross-Year Match'

                common_data = {
                    'Match ID': match_id, 'Status': status, 'Cross-Year': cross_year,
    ...
    def _process_tax_based_matches(self):
        ...
        for idx, book_row in self.books_df[self.books_df['processed'] == False].iterrows():
            ...
            # Find potential matches in GSTR-2A
            gstr2a_matches = self.gstr2a_df[
                (self.gstr2a_df['processed'] == False) &
                (abs(self.gstr2a_df['Total IGST Amount'] - book_row['Total IGST Amount']) <= self.tax_tolerance) &
                (abs(self.gstr2a_df['Total CGST Amount'] - book_row['Total CGST Amount']) <= self.tax_tolerance) &
                (abs(self.gstr2a_df['Total SGST Amount'] - book_row['Total SGST Amount']) <= self.tax_tolerance)
            ]
            ...
            if len(gstr2a_matches) > 0:
                ...
                for _, gstr2a_row in gstr2a_matches.iterrows():
                    ...
                    if book_sign == gstr2a_sign:
                        ...
                        # Calculate score with higher weight for GSTIN and name similarities
                        score = 0
                        if pd.notna(date_diff) and date_diff <= self.date_tolerance.days:
                            score += 20
                        score += (gstin_similarity * 0.5)  # 50% weight for GSTIN
                        score += (legal_name_similarity * 0.25)  # 25% weight for legal name
                        score += (trade_name_similarity * 0.25)  # 25% weight for trade name
                ...
                if valid_matches:
                    ...
                    # Additional validation: require minimum score threshold
                    ...
                    # Calculate GSTIN and name similarities
                    gstin_similarity = 0
    ...
    def _generate_final_report(self):
        ...
        all_expected_columns = [
            ...
            'IGST Diff', 'CGST Diff', 'SGST Diff', 'Date Diff', 'Status', 'Tax Diff Status', 'Date Status',
            'Tax Sign Status', 'Value Sign', 'Narrative', 'Suggestions'
        ]

        # Add missing columns to self.matched_df with NaN or appropriate default values
        for col in all_expected_columns:
            if col not in self.matched_df.columns:
                self.matched_df[col] = np.nan if 'Diff' in col or 'Value' in col else ''  # Default based on column type expectation

        # Add missing columns to self.mismatch_df with NaN or appropriate default values
        for col in all_expected_columns:
            if col not in self.mismatch_df.columns:
                self.mismatch_df[col] = np.nan if 'Diff' in col or 'Value' in col else ''
    ...
    def _generate_recon_summary(self):
        ...
        # Total for Reconciliation Summary
        total_books_count_recon = len(final_books)
        total_gstr2a_count_recon = len(final_gstr2a)
    ...
    def run_enhanced_group_matching(self):
        ...
        for key in set(books_groups.groups.keys()).union(gstr2a_groups.groups.keys()).union(partials_groups.groups.keys()):
            ...
            if (abs(total_books_tax['Total IGST Amount'] - total_gstr2a_tax['Total IGST Amount']) <= self.group_tax_tolerance and abs(total_books_tax['Total CGST Amount'] - total_gstr2a_tax['Total CGST Amount']) <= self.group_tax_tolerance and abs(total_books_tax['Total SGST Amount'] - total_gstr2a_tax['Total SGST Amount']) <= self.group_tax_tolerance):
                ...
                for idx in books_group.index:
                    mismatch_updates.append({'idx': idx, 'Status': 'Group Match', 'Group ID': group_id, 'Match ID': int(match_id), 'IGST Diff': float(igst_diff), 'CGST Diff': float(cgst_diff), 'SGST Diff': float(sgst_diff), 'Tax Diff Status': tax_diff_status, 'Narrative': narration, 'Suggestions': suggestions})
                for idx in gstr2a_group.index:
                    mismatch_updates.append({'idx': idx, 'Status': 'Group Match', 'Group ID': group_id, 'Match ID': int(match_id), 'IGST Diff': float(igst_diff), 'CGST Diff': float(cgst_diff), 'SGST Diff': float(sgst_diff), 'Tax Diff Status': tax_diff_status, 'Narrative': narration, 'Suggestions': suggestions})
                for idx in partials_books.index:
                    matched_updates.append({'idx': idx, 'Status': 'Group Match', 'Group ID': group_id, 'Match ID': int(match_id), 'IGST Diff': float(igst_diff), 'CGST Diff': float(cgst_diff), 'SGST Diff': float(sgst_diff), 'Tax Diff Status': tax_diff_status, 'Narrative': narration, 'Suggestions': suggestions})
