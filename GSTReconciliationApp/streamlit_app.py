...
from utils.reports import render_unique_gst_report, get_report_summary, render_enhanced_insights, get_unmapped_gst_summary_for_excel, add_gstr2a_compliance_columns, get_gstr2a_compliance_summary
import io
import openpyxl
import time
...
# --- Main Content Tabs (only show if not on Settings page) ---
if section != "Settings":
    ...
    with tabs[1]:
        ...
        with transaction_tabs[0]:
            if st.session_state.get('reconciliation_complete') and st.session_state.get('final_report') is not None:
                ...
                required_cols = [
                    'IGST Diff', 'CGST Diff', 'SGST Diff', 'Date Diff', 'Status', 'Tax Diff Status',
                ...
                partial_group_cellstyle = JsCode('''
                    function(params) {
                        ...
                        if (status == 'Partial Match' || status == 'Group Match') {
                            // Highlight diff columns if not No Difference/Within Tolerance
                            if (
                                (params.colDef.field == 'IGST Diff' && taxDiff == 'Has Difference') ||
                                (params.colDef.field == 'CGST Diff' && taxDiff == 'Has Difference') ||
                                (params.colDef.field == 'SGST Diff' && taxDiff == 'Has Difference') ||
                                (params.colDef.field == 'Tax Diff Status' && taxDiff == 'Has Difference') ||
                            ...
                            if (params.colDef.field == 'Total IGST Amount' && Math.abs(igstDiff) > taxTolerance && taxDiff == 'Has Difference') {
                                return { 'backgroundColor': '#fffbe6', 'fontWeight': 'bold' };
                            }
                            if (params.colDef.field == 'Total CGST Amount' && Math.abs(cgstDiff) > taxTolerance && taxDiff == 'Has Difference') {
                                return { 'backgroundColor': '#fffbe6', 'fontWeight': 'bold' };
                            }
                            if (params.colDef.field == 'Total SGST Amount' && Math.abs(sgstDiff) > taxTolerance && taxDiff == 'Has Difference') {
                                return { 'backgroundColor': '#fffbe6', 'fontWeight': 'bold' };
                            }
